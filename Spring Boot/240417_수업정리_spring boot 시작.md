...
- request.getParmeter( ) 를 통해 파라미터 값을 가져오고, 그것을 또 setter 메소드를 통해 필드에 값을 넣어주고, 마지막으로 그 객체를 mybatis parameter로 보내서 DB와 관련된 작업을 하는.. 이런 일련의 코드들이 필요 없어진다.
![](../image/Pasted%20image%2020240417162318.png)
![](../image/Pasted%20image%2020240417162416.png)
📌command 객체?
- 요청 파라미터 값을 저장하는 객체. VO(DTO)와 같다고 생각할 수 있을 것 같다.
...
- 파라미터 이름 age로 오는 값을 정수형 변수 age에 저장하라는 의미
- String으로 넘어오는 파라미터의 값을 변수 자료형에 맞추어 자동으로 캐스팅한다.
...
📌오류 수정 방법 : age2라는 파라미터가 없는 경우 required=false로 하고, 기본값으로 10(임의의 값)을 설정한다.
- (@RequestParam(value="age2", defaultValue="10", required=false) int age, Model model)

#### 파라미터가 항상 넘어오는 경우(파라미터와 같은 이름의 변수 활용)
