...
- 스프링에서 Model 객체는 컨트롤러에서 생성된 데이터를 담아서 view 페이지에 전달하는 역할을 한다.
...
📌①input 태그의 name 속성 ②프로퍼티 (③테이블 컬럼-mybatis이용할 때) 일치시켜주면
- request.getParmeter( ) 를 통해 파라미터 값을 가져오고, 그것을 또 setter 메소드를 통해 객체에 ..
