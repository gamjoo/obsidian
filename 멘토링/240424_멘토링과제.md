...
- 객체 자료형을 비교할 때 주소 값이 아닌 객체의 필드값을 기준으로 동등 비교 기준을 변경하고 싶으면, equals() 메소드를 오버라이딩 하여 주소가 아닌 필드값을 비교하도록 재정의한다.
- java.util 패키지에 있는 Objects 클래스도 활용한다.
![](../image/Pasted%20image%2020240423184510.png)
```java
...
class Person {
    ...
    // 객체 주소 비교가 아닌 Person 객체의 name필드의 값이 동등한지 비교하기 위해 오버라이딩
    public boolean equals(Object obj) {
        if (this == obj)
        return true; // 만일 메소드를 호출하고 있는 객체(this)와 매개변수로 입력한 객체의 주소값이 같은 경우 true
        if (!(obj instanceof Person))
        return false; // 만일 매개변수 객체가 Person 타입과 호환되지 않으면 false
        Person person = (Person) obj; // 만일 매개변수 객체가 Person 타입과 호환된다면 다운캐스팅(down casting) 진행
        return Objects.equals(this.name, person.name); // 메소드를 호출하고 있는 객체(this)의 name필드와 매개변수 객체의 name필드값이 같을경우 true, 다를 경우 false
    }
}
...
- equals 메소드와 hashcode 메소드, 두 메소드를 ove
