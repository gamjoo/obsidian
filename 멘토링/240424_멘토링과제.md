...
class Person {
    ...
    public boolean equals(Object obj) {
        if (this == obj) return true; // 만일 메서드를 호출하고 있는 객체(this)와 매개변수로 입력한 객체의 주소값이 같은 경우 true
        if (!(obj instanceof Person)) return false; // 만일 매개변수 객체가 Person 타입과 호환되지 않으면 false
