...
- 처음 선언한 배열의 크기(5)는 변경할 수 없다(정적 할당;static allocation)
...
- 길이가 가변적이다(동적 할당;dynamic allocation)
- 데이터들이 연속적으로 나열된다. (메모리에 연속적으로 나열되지 않고, 각 데이터들은 주소(reference)로 연결되어 있다)
