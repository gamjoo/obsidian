...
- 객체 자료형을 비교할 때 주소 값이 아닌 객체의 필드값을 기준으로 동등 비교 기준을 변경하고 싶으면, equals() 메서드를 오버라이딩 하여 주소가 아닌 필드값을 비교하도록 재정의한다.
- java.util 패키지에 있는 Object
...
class Person {
    ...
    public boolean equals(Object obj) {
        if (this == obj)
        return true; // 만일 메서드를 호출하고 있는 객체(this)와 매개변수로 입력한 객체의 주소값이 같은 경우 true
        ...
        return Objects.equals(this.name, person.name); // 메서드를 호출하고 있는 객체(this)의 name필드와 매개변수 객체의 name필드값이 같을경우 true, 다를 경우 false
    }
}
